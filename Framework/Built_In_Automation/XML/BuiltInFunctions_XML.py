...
update_tag_list = [
    ...
    "replace",
    "REPLACE",
]  # Update/replace existing element(s)
delete_tag_list = [
    "Delete",
    "delete",
    "DELETE",
    ...
    "remove",
    "REMOVE",
]  # Delete existing line or element(s)
    ...
    try:
        if len([step_data]) != 1:  # Verifies that length of step_data greater than one
            ...
            if (element_step_data == []) or (element_step_data in failed_tag_list):
                ...
                returned_element = get_target_element(
                    element_step_data[0],
                    element_step_data[1],
                    element_step_data[2],
                    element_step_data[3],
                ...
                if (returned_element == []) or (
                    returned_element in failed_tag_list
                ):  # if failed to update target elements
                    CommonUtil.ExecLog(
                        sModuleInfo,
                        "Unable to change the attribute value of '%s'"
                        % element_step_data[0],
                        3,
                    )
                    return "zeuz_failed"
                ...
                ):  # if target elements are same as expected elements
                    CommonUtil.ExecLog(
                        sModuleInfo,
                        "Nothing to change the attribute value of '%s'"
                        % element_step_data[0],
                        1,
                    )
                    return "Passed"
                    ...
                    CommonUtil.ExecLog(
                        sModuleInfo,
                        "Updated the target attribute value of '%s'"
                        % element_step_data[0],
                        1,
                    )
                    return "Passed"
    ...
    try:
        if len([step_data]) != 1:  # Verifies that length of step_data greater than one
            ...
            if (element_step_data == []) or (element_step_data in failed_tag_list):
                ...
                CommonUtil.ExecLog(
                    sModuleInfo, "File tree of: %s" % (element_step_data[0]), 1
                )
                CommonUtil.ExecLog(
                    sModuleInfo, "%s" % ET.tostring(returned_element[0]), 1
                )
                return "Passed"
        ...
        if len(step_data) >= 1:
            for each in step_data:
                if each[1] == "path":
                    ...
                    CommonUtil.ExecLog(
                        sModuleInfo, "Unable to find elements requested.", 3
                    )
...
def get_target_element(
    file_path, target_parameter, target_value, action_name, action_value, step_data
):
    """
    ...
    try:
        ...
        CommonUtil.ExecLog(
            sModuleInfo, ">>> The expected attribute value is: '%s'" % action_value, 1
        )
...
def update_target_element(
    ...
    try:
        ...
        if current_value == action_value:
            CommonUtil.ExecLog(
                sModuleInfo,
                ">>> The current attribute value '%s' is already set to the target value."
                % current_value,
                1,
            )
            return "Default"
            ...
            CommonUtil.ExecLog(
                sModuleInfo,
                ">>> The current attribute value is: '%s'" % current_value,
                1,
            )
            ...
            CommonUtil.ExecLog(
                sModuleInfo,
                ">>> Unable to find the target attribute. Please check the data set(s).",
                1,
            )
        ...
        if action_name == "update":
            ...
            CommonUtil.ExecLog(
                sModuleInfo, ">>> Updating the attribute value in the XML File...", 1
            )

        else:
            CommonUtil.ExecLog(
                sModuleInfo,
                ">>> Unable to find the action name. Please check the data set(s).",
                1,
            )
        ...
        CommonUtil.ExecLog(
            sModuleInfo,
            ">>> The current attribute value updated to the expected value '%s'"
            % action_value,
            1,
        )
...
def update_action_value(file_path, doc):
    ...
    try:
        ...
        doc.write(file_path)
        CommonUtil.ExecLog(
            sModuleInfo, ">>> Writing the attribute value in the XML File...", 1
        )
