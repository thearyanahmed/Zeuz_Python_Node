...
def Check_uncheck(data_set):
    ...
    if "Toggle" in pattern_list:
        is_selected = Element.GetCurrentPattern(
...
def _child_bfs_search(
    ...
    try:
        ...
        while q:
            ...
            found = True
            if found and element_name is not None and not _found(element_name, NameE):
                found = False
            if (
                found
                and element_class is not None
                and not _found(element_class, ClassE)
            ):
                found = False
            if (
                found
                and element_automation is not None
                and not _found(element_automation, AutomationE)
            ):
                found = False
            if (
                found
                and element_control is not None
                and not _found(element_control, LocalizedControlTypeE)
            ):
                found = False
            if found:
...
def _child_search(
    ...
    try:
        ...
        if (element_name, element_class, element_automation, element_control) == (
            None,
            None,
            None,
            None,
        ...
        if found and element_name is not None and not _found(element_name, NameE):
            found = False
        if found and element_class is not None and not _found(element_class, ClassE):
            found = False
        if (
            found
            and element_automation is not None
        ...
        if found:  # 1st method
            ...
            if len(all_elements) - 1 == element_index:
                if code_debug:
                    print(
                        'name="%s", controlType="%s", automationId="%s", class="%s"'
...
def _child_search_with_parent(
    ...
    try:
        ...
        if not parent_found:
            found = True
            if found and parent_name is not None and not _found(parent_name, NameE):
                found = False
            if found and parent_class is not None and not _found(parent_class, ClassE):
                found = False
            if (
                found
                and parent_automation is not None
                and not _found(parent_automation, AutomationE)
            ):
                found = False
            if (
                found
                and parent_control is not None
                and not _found(parent_control, LocalizedControlTypeE)
            ):
                found = False
            parent_found = found
...
def _child_search_with_parent_sibling(
    ...
    try:
        ...
        if not parent_found:
            ...
            found = True
            if found and sibling_name is not None and not _found(sibling_name, NameE):
                found = False
            if (
                found
                and sibling_class is not None
                and not _found(sibling_class, ClassE)
            ):
                found = False
            if (
                found
                and sibling_automation is not None
                and not _found(sibling_automation, AutomationE)
            ):
                found = False
            if (
                found
                and sibling_control is not None
                and not _found(sibling_control, LocalizedControlTypeE)
            ):
                found = False
...
def _element_path_parser(element_path: str):
    try:
        ...
        while True:
            ...
            element_path = element_path[element_path.find("=") + 1 :].strip()
            att_value = (
                re.findall("\d+", element_path)[0].strip()
                if "index" in attribute
                else re.findall("('[^']*'|\"[^\"]*\")", element_path)[0].strip()
            )
            ...
            if "index" in attribute and element_path[
                element_path.find(att_value) - 1
            ] in ("'", '"'):
                element_path = element_path[len(att_value) + 2 :].strip()
            else:
                element_path = element_path[len(att_value) :].strip()
            if "index" not in attribute:
                att_value = att_value[1:-1]

            if "class" in attribute:
                element_class = [att_value, _count_star(attribute)]
            elif "name" in attribute:
                element_name = [att_value, _count_star(attribute)]
            elif "automation" in attribute:
                element_automation = [att_value, _count_star(attribute)]  # automationid
            elif "control" in attribute:
                element_control = [
                    att_value,
                    _count_star(attribute),
                ]  # localizedcontroltype
            elif "index" in attribute:
...
def Element_path_search(window_name, element_path):
    try:
        ...
        if window_name is None:
            ...
            CommonUtil.ExecLog(
                sModuleInfo,
                'Window="%s" is selected. We are stepping inside the Window and will search for:\n%s'
                % (ParentElement.Current.Name, element_path[:-1]),
                1,
            )
        global tabs
        tabs = 0
...
def _child_search_by_path(
    ParentElement, element_path, sModuleInfo, switch_window=False
):
    element_name, element_class, element_automation, element_control = (
        None,
        None,
        None,
        None,
    ...
    try:
        (
            ...
            element_class,
            element_index,
        ) = _element_path_parser(element_path)
        if exact == "error":
            return []
        if (element_name, element_class, element_automation, element_control) == (
            None,
            ...
            CommonUtil.ExecLog(
                sModuleInfo,
                "Element information is missing in a potion of Element Path",
                3,
            )
            return []
        if element_index < 0:
            CommonUtil.ExecLog(
                sModuleInfo,
                "Please provide positive index in case of Element Path search",
                2,
            )
        if exact:
            child_elements = ParentElement.FindAll(
            ...
            if child_elements.Count == 0:
                CommonUtil.ExecLog(
                    sModuleInfo,
                    "The following element has no child:\n"
                    + 'Name="%s", ControlType="%s", Class="%s", AutomationId="%s"'
                    % (
                        ParentElement.Current.Name,
                        ParentElement.Current.LocalizedControlType,
                        ParentElement.Current.ClassName,
                        ParentElement.Current.AutomationId,
                    ),
                    3,
                )
                return []
            ...
            for each_child in child_elements:
                ...
                if (
                    ...
                    and not _found(element_name, NameE)
                ):
                    found = False
                if (
                    found
                ...
                if found:
                    if switch_window:
                        CommonUtil.ExecLog(
                            sModuleInfo, "Switching to window: %s" % NameE, 1
                        )
            ...
            if element_path:
                return _child_search_by_path(
                    all_elements[element_index], element_path, sModuleInfo
                )
            else:
                return [all_elements[element_index]]
            ...
            if element_path:
                return _child_search_by_path(
                    temp[element_index], element_path, sModuleInfo
                )
            else:
    ...
    try:
        if image_text:
            ...
            pytesseract.tesseract_cmd = (
                os.environ["PROGRAMFILES"] + r"\Tesseract-OCR\tesseract.exe"
            )

            image_text = image_text.replace(" ", "").lower()
            PIL.ImageGrab.grab().crop((left, top, left + width, top + height)).save(
                "sample.jpg"
            )
            ...
            if -len(all_pos) <= idx < len(all_pos):
                CommonUtil.ExecLog(
                    sModuleInfo,
                    "Found %s text elements. Returning element of index %s"
                    % (len(all_pos), idx),
                    1,
                )
                i = all_pos[idx]
                ...
                CommonUtil.ExecLog(
                    sModuleInfo,
                    "Found %s text elements. Index out of range" % len(all_pos),
                    3,
                )
                return "zeuz_failed"
            else:
                CommonUtil.ExecLog(
                    sModuleInfo, 'Could not find text "%s"' % image_text, 3
                )
                return "zeuz_failed"
            ...
            center = (
                left + (right_bottom[0] + left_top[0]) // 2,
                top + height - (right_bottom[1] + left_top[1]) // 2,
            )
            ...
            if match is not None:  # Match found, so scale
                CommonUtil.ExecLog(
                    sModuleInfo, "Scaling image (%s)" % match.group(0), 5
                )
                size_w, size_h = (
                    int(match.group(1)),
                    int(match.group(2)),
                )  # Extract width, height from match (is screen resolution of desktop image was taken on)
                file_name = _scale_image(
                    file_name, size_w, size_h
                )  # Scale image element
            ...
            if -len(element_list) <= idx < len(element_list):
                CommonUtil.ExecLog(
                    sModuleInfo,
                    "Found %s elements. Returning element of index %s"
                    % (len(element_list), idx),
                    1,
                )
...
def Get_Element(
    ...
    parent_name, parent_class, parent_automation, parent_control, parent = (
        None,
        None,
        None,
        None,
    ...
    parent_path = ""
    sibling_name, sibling_class, sibling_automation, sibling_control, sibling = (
        None,
        None,
        None,
        None,
    ...
    try:
        for left, mid, right in data_set:
            ...
            if mid == "element parameter":
                ...
                elif "automation" in left:
                    element_automation = [right, _count_star(left)]  # automationid
                elif "control" in left:
                    element_control = [right, _count_star(left)]  # localizedcontroltype
                elif "path" in left:
                    element_path = right.strip()
                elif "image" in left:
                ...
                parent = True
                if "class" in left:
                    parent_class = [right, _count_star(left)]
                elif "name" in left:
                    parent_name = [right, _count_star(left)]
                elif "automation" in left:
                    parent_automation = [right, _count_star(left)]  # automationid
                elif "control" in left:
                    parent_control = [right, _count_star(left)]  # localizedcontroltype
                elif "path" in left:
                    parent_path = right.strip()
                elif "index" in left:
                    parent_index = right.strip()

            elif mid == "sibling parameter":
                sibling = True
                if "class" in left:
                    sibling_class = [right, _count_star(left)]
                elif "name" in left:
                    sibling_name = [right, _count_star(left)]
                elif "automation" in left:
                    sibling_automation = [right, _count_star(left)]  # automationid
                elif "control" in left:
                    sibling_control = [right, _count_star(left)]  # localizedcontroltype
        ...
        if elem and (
            ...
            CommonUtil.ExecLog(
                sModuleInfo,
                "We support only 'Window', 'Name', 'ClassName', 'LocalizedControlType', 'AutomationId', 'element path', 'image'",
                3,
            )
            return "zeuz_failed"
        ...
        if (
            ...
            CommonUtil.ExecLog(
                sModuleInfo,
                "For image parent, We support only 'Window', 'Name', 'ClassName', 'LocalizedControlType', 'AutomationId', 'path'",
                3,
            )
            return "zeuz_failed"
        elif (
            parent
            and not element_image
            and (parent_name, parent_class, parent_automation, parent_control)
            == (None, None, None, None)
        ):
            CommonUtil.ExecLog(
                sModuleInfo,
                "We support only 'Window', 'Name', 'ClassName', 'LocalizedControlType', 'AutomationId'",
                3,
            )
            return "zeuz_failed"
        if sibling and (
        ...
        if window_name is None and Parent_Element is None and not element_path:
            CommonUtil.ExecLog(
                sModuleInfo,
                "You should provide 'Window' otherwise the search won't be efficient",
                2,
            )
        if element_path:
            if element_path[-1] != ">":
            ...
            if element_index != 0:
                CommonUtil.ExecLog(
                    sModuleInfo,
                    "Index is not allowed other than 0 for 'element path' search. Setting index = 0",
                    2,
                )
        ...
        while True:
            if element_image:
                ...
                for i in (
                    ("name", parent_name),
                    ("class", parent_class),
                    ("automationid", parent_automation),
            ...
            if (
                all_elements != "zeuz_failed"
                and -len(all_elements) <= element_index < len(all_elements)
                or time.time() > s + wait_time
            ):
                break
        ...
        if -len(all_elements) <= element_index < len(all_elements):
            if element_index == 0:
                CommonUtil.ExecLog(
                    sModuleInfo, "Returning the 1st element that is found", 1
                )
            else:
                CommonUtil.ExecLog(
                    sModuleInfo,
                    "Returning the element of index = %d" % element_index,
                    1,
                )
        else:
        ...
        for MainWindowElement in MainWindowsList:
            try:
                ...
                if _found(WindowName, NameS):
                    CommonUtil.ExecLog(
                        sModuleInfo, "Switching to window: %s" % NameS, 1
                    )
    ...
    try:
        for left, mid, right in data_set:
            ...
            if "src" in left or "source" in left:
                source.append(
                    (left.replace("src", "").replace("source", ""), mid, right)
                )
            elif "dst" in left or "destination" in left:
                destination.append(
                    (left.replace("dst", "").replace("destination", ""), mid, right)
                )
...
def Save_Attribute(data_set):
    ...
    try:
        ...
        pattern_list = [
            Automation.PatternName(i) for i in Element.GetSupportedPatterns()
        ]
        print(pattern_list)
        ...
        if "value" in field:
            if "Value" not in pattern_list:
                CommonUtil.ExecLog(
                    sModuleInfo, "Value pattern is not found for this Element", 3
                )
                return "zeuz_failed"
            actual_text = str(
            ...
            if "Toggle" not in pattern_list:
                CommonUtil.ExecLog(
                    sModuleInfo, "Toggle pattern is not found for this Element", 3
                )
                return "zeuz_failed"
            actual_text = (
                True
                if Element.GetCurrentPattern(TogglePattern.Pattern).Current.ToggleState
                else False
            )
        elif "select" in field and "pattern" in field:
            if not "SelectionItem" in pattern_list:
                CommonUtil.ExecLog(
                    sModuleInfo, "SelectionItemPattern is not found for this Element", 3
                )
                return "zeuz_failed"
            actual_text = Element.GetCurrentPattern(
    ...
    try:
        ...
        for left, mid, right in data_set:
            if mid.lower().strip() == "action":
                text = right
            elif left.lower().strip() == "method" and right.lower().strip() in (
                "gui",
                "keystroke",
            ):
                keystroke = True
        ...
        if keystroke:
            ...
            try:
                ...
                CommonUtil.ExecLog(
                    sModuleInfo,
                    "Retrying with autoit. Yet if it does not work please try 'keystroke' as method instead",
                    2,
                )
                # return "zeuz_failed"
...
def Swipe(data_set):
    try:
        ...
        CommonUtil.ExecLog(
            sModuleInfo,
            "Scrolled %s the window element %s times" % (direction, scroll_count),
            1,
        )
        return "passed"
    except Exception:
        return CommonUtil.Exception_Handler(
            sys.exc_info(), None, "Can't scroll the given window element."
        )
...
def Scroll_to_element(dataset):
    try:
        ...
        if desired_Element != "zeuz_failed":
            CommonUtil.ExecLog(
                sModuleInfo, "Desired element is found.No need to scroll.", 1
            )
        else:
            count = 0
            while True:
            ...
            CommonUtil.ExecLog(
                sModuleInfo,
                "Scrolled %s the window element %s times"
                % (direction, scroll_count * count),
                1,
            )
        time.sleep(unnecessary_sleep)
        return "passed"
...
def _open_inspector(inspector, args):
    d = {
        "uispy": ["UI Spy", "UISpy"],
        "inspectx64": ["Inspect  (HWND", "InspectX64"],
        "inspectx86": ["Inspect  (HWND", "InspectX86"],
    }
    ...
    try:
        ...
        if not launch_cond and len(pygetwindow.getWindowsWithTitle(Desktop_app)) > 0:
            CommonUtil.ExecLog(
                ...
                + 'If you want to keep the existing app and launch another instance of it then add a row ("launch again", "optional parameter", "yes")',
                2,
            )
        else:
            ...
            CommonUtil.ExecLog(
                sModuleInfo,
                "Waiting for the app to launch for maximum %s seconds" % wait,
                1,
            )
            s = time.time()
            while time.time() - s < wait:
                ...
                if (
                    Desktop_app in pygetwindow.getActiveWindow().title
                ...
                if maximize or size is not None:
                    CommonUtil.ExecLog(
                        sModuleInfo,
                        "Could not find any launched app with title: %s however continuing. Maximize or custom app size wont work"
                        % Desktop_app,
                        2,
                    )
                else:
                    CommonUtil.ExecLog(
                        sModuleInfo,
                        "Could not find any launched app with title: %s however continuing"
                        % Desktop_app,
                        2,
                    )
                return "passed"
        ...
        if maximize:
            ...
            CommonUtil.ExecLog(
                sModuleInfo,
                "Maximizing your app. Your Desktop Resolution = %s"
                % str((gui.size()[0], gui.size()[1])),
                1,
            )
            ...
            CommonUtil.ExecLog(
                sModuleInfo,
                "Restoring and Resizing the app with Size = %s and Top_Left = %s"
                % (size, top_left),
                1,
            )
            win = pygetwindow.getWindowsWithTitle(Desktop_app)[0]
        ...
        if method_name == "pyautogui":
            try:
                if keystroke_char != "":
                    ...
                    CommonUtil.ExecLog(
                        sModuleInfo,
                        "Successfully entered characters with pyautogui:\n%s"
                        % keystroke_char,
                        1,
                    )
                    return "passed"
            ...
            for i in range(count):
                gui.hotkey(
                    *keys
                )  # Send keypress (as individual values using the asterisk)
            ...
            try:
                if keystroke_char != "":
                    ...
                    CommonUtil.ExecLog(
                        sModuleInfo,
                        "Successfully entered character with autoit:\n%s"
                        % keystroke_char,
                        1,
                    )
                    return "passed"
...
def save_attribute_values_in_list(data_set):
    ...
    try:
        ...
        if Element == "zeuz_failed":
            CommonUtil.ExecLog(
                sModuleInfo,
                "Could not find the main Element. We are searching for targets through out the whole desktop",
                2,
            )
        ...
        try:
            for left, mid, right in data_set:
                ...
                if "target parameter" in mid:
                    ...
                    paired = (
                        False
                        if right.lower() in ("no", "false") or "colum" in right.lower()
                        else True
                    )
                    p = True
                elif left == "scroll count":
                    ...
                    for elem in each:
                        if search_by_attribute.strip().lower() == "value":
                            Attribute_value = eval(
                                "elem.GetCurrentPattern(ValuePattern.Pattern).Current.Value"
                            )
                        else:
                            Attribute_value = eval(
                                "elem.Current." + _get_attribute(search_by_attribute)
                            )
                        try:
                            for search_contain in target[i][2]:
                                if (
                                    not isinstance(
                                        search_contain, type(Attribute_value)
                                    )
                            ...
                            for search_doesnt_contain in target[i][3]:
                                if (
                                    isinstance(
                                        search_doesnt_contain, type(Attribute_value)
                                    )
